...
public class ResponseHandler {
  protected static void rewriteResponse(Response response, List<HostRewriter> hostRewriters)
    ...
    for (HostRewriter hostRewriter : hostRewriters) {
      try {
        hostRewriter.rewriteResponse(requestMethod_SB, requestURI_SB, requestHeaders,
            responseHeaders);
      } catch (IOException e) {
        continue;
      }
  ...
  protected static byte[] processResponse(HttpExchange httpExchange, HttpURLConnection connection,
  ...
  protected static void handleResponse(Response response, List<HostHandler> hostHandlers,
    ...
    for (HostHandler hostHandler : hostHandlers) {
      try {
        hostHandler.handleResponse(response.requestMethod, response.requestURI, requestHeaders,
            responseHeaders, bufferInput);
      } catch (IOException e) {
        continue;
      }
    }
