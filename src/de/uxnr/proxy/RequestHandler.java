...
public class RequestHandler {
  protected static void rewriteRequest(Request request, List<HostRewriter> hostRewriters)
  ...
  protected static byte[] processRequest(HttpExchange httpExchange, HttpURLConnection connection,
  ...
  protected static void handleRequest(Request request, List<HostHandler> hostHandlers, byte[] body)
      throws IOException {
    ...
    for (HostHandler hostHandler : hostHandlers) {
      try {
        hostHandler.handleRequest(request.requestMethod, request.requestURI, requestHeaders,
            bufferInput);
      } catch (IOException e) {
        continue;
      }
